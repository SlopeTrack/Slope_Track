...
dataset = 'slopestrack/val'

val_pred = 'oracle_analysis/val_appearance_slopestrack/data'
if not os.path.exists(val_pred):
    os.makedirs(val_pred)

val_seqs = [f for f in sorted(os.listdir(dataset)) if f.endswith('.txt')] #sorted(os.listdir(dataset))[args.start:args.end+1]
for video_name in val_seqs:
    print(video_name)
    det_results = {}
    with open(os.path.join(dataset, video_name), 'r') as f:
...
#     star_idx = len(gb.glob(os.path.join(dataset, video_name, 'img1') + "/*.jpg")) // 2 + 1
    ...
    gt_file = base_name + '.mp4'
    cap = cv2.VideoCapture(os.path.join(dataset, gt_file))
    i = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
...
#         image_path = os.path.join(dataset, video_name, 'img1', '{:0>6d}.jpg'.format(frame_id + star_idx))
        ...
        results.append((i, online_tlwhs, online_ids))
...
# dataset = 'mot/val'
dataset = 'sportsmot_val'

val_pred = 'oracle_analysis/sportsmot_val_appearance'
if not os.path.exists(val_pred):
    os.makedirs(val_pred)
