...
dataset='dancetrack_val'

val_pred = 'oracle_analysis/dancetrack_val_sort'
if not os.path.exists(val_pred):
    os.makedirs(val_pred)

#MOT
#val_seqs = [f for f in sorted(os.listdir(source_dir)) if 'FRCNN' in f]
val_seqs = sorted(os.listdir(dataset))
for video_name in val_seqs:
    print(video_name)
    det_results = {}
    # For MOT series
    # with open(os.path.join(dataset, video_name,'gt/gt_val_half.txt'), 'r') as f:
