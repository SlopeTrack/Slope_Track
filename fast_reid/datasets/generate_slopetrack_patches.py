...
def make_parser():
    ...
    parser.add_argument("--data_path", default="", help="path to SlopeTrack data")
    parser.add_argument("--save_path", default="fast_reid/datasets", help="Path to save the SlopeTrack-ReID dataset")
...
def main(args):
    ...
    save_path = os.path.join(args.save_path, 'SlopeTrack' + '-ReID')
    ...
    for seq in seqs:        # iteration over seqs
        print("current seq", seq, seq_num)
        print("current id_offset", id_offset)

        ground_truth_path = os.path.join(data_path, seq, 'gt/gt.txt')
        gt = generate_trajectories(ground_truth_path, GroundTrues=False)  # [do filter] frame, id, x_tl, y_tl, x_br, y_br, active, category, visible_ratio
        ...
        for f in tqdm(range(num_frames)):     # iteration over frames
            ...
            for d in range(np.size(det, 0)):
                ...
                fileName = (str(id_+id_offset)).zfill(7) + '_' + str(seq_num) + '_' + (str(f+1)).zfill(7) + '_acc_data.bmp'
