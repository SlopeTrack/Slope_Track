...
def make_parser():
    ...
    parser.add_argument("--dataset", type=str, default="slopetrack")
    parser.add_argument("--result_folder", type=str, default="results/trackers/")
    parser.add_argument("--test_folder", type=str, default="/home/cm105147/datasets/test")
    ...
    # reid args #Unused here replicate 
    ...
    parser.add_argument("--emb_off", action="store_true", default=True)
    parser.add_argument("--cmc_off", action="store_true", default=True)
    parser.add_argument("--aw_off", action="store_true", default=True)
    parser.add_argument("--aw_param", type=float, default=0.5)
    parser.add_argument("--new_kf_off", action="store_true", default=True)
    parser.add_argument("--grid_off", action="store_true", default=True)
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            if args.with_reid:
                ...
                pred = outputs
                #print(img_info)
                online_targets = tracker.update(pred, img_info['raw_img'], img_info['raw_img'], tag) # update(outputs, [img_info['height'], img_info['width']])
            online_tlwhs = []
            online_ids = []
...
def main(args):
    ...
    if args.save_result:
        vis_folder = osp.join(output_dir, 'deep_oc_sort')
