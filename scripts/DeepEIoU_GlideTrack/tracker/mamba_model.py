...
class MambaPositionPredictor(nn.Module):
    ...
    def forward(self, x, lengths, target_len, nll=False):
            """
            Args:
            x: (B, T_in, in_dim)
            ...
            h_original = self.dropout1(self.in_proj(new_enc_in))  # (B, T_in, d_model)
            h = h_original
            ...
            if nll:
               ...
               return mu_all[:,:target_len,:], sigma_all[:,:target_len,:]
            #sigma_all.append(sigma)
            else:
               mu_all = self.head(output)
               return mu_all[:,:target_len,:]
            #print(mu_all)
