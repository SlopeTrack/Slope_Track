...
def normalize_sequence(positions, option, device="cpu"):
    ...
    if option == 4:
        ...
        cumsum = np.cumsum(h[1:])  # cumulative sum excluding the first element
        time = np.arange(2, h.shape[0] + 1).reshape(-1, 1)  # time steps from 2 to seq_len
        moving_avg_h = (cumsum / time.squeeze())  # shape: [seq_len - 1]

        cumsum = np.cumsum(w[1:])
        time = np.arange(2, w.shape[0] + 1).reshape(-1, 1)
        moving_avg_w = cumsum / time.squeeze()
        ...
        delta_w = np.concatenate(([0.0], moving_avg_w), axis=0)
        delta_h = np.concatenate(([0.0], moving_avg_h), axis=0)
