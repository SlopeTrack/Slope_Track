...
def make_parser():
    parser = argparse.ArgumentParser("GeneralTrack")
    ...
    # tracking args
    parser.add_argument("--byte", default=True)
    parser.add_argument("--track_thresh", type=float, default=0.5, help="tracking confidence threshold")
    parser.add_argument("--det_thresh", type=float, default=0.6, help="tracking confidence threshold")
    ...
    parser.add_argument('--restore_ckpt', default='/home/cm105147/code/GeneralTrack/checkpoints/generaltrack_bdd.pth', help="restore checkpoint")
    ...
    parser.add_argument('--roialign_size', default=(2, 2))  # （h, w）
    parser.add_argument('--corr_radius', type=int, default=4, help="原来[400, 720]都是4，[720, 1280]是7")
...
class Predictor(object):
    def __init__(
        ...
        if self.with_reid:
            ...
            self.encoder = FastReIDInterface(self.fast_reid_config, self.fast_reid_weights, device='cpu')
    ...
    def inference(self, img, frame_id):
        ...
        outputs = np.concatenate((bboxes, scores.reshape(-1, 1), classes.reshape(-1, 1)), axis=1)
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            if args.with_reid:
                ...
                frame_rgb = cv2.cvtColor(img_info['raw_img'], cv2.COLOR_BGR2RGB)
                frame_cuda = torch.from_numpy(img_info['raw_img']).permute(2,0,1).float()
                frame_cuda = frame_cuda.unsqueeze(0)
                frame_cuda = frame_cuda.to('cuda')
                print(frame_cuda.shape)
                online_targets = tracker.update(outputs, [img_info['height'], img_info['width']], [img_info['height'], img_info['width']], frame_cuda)
            online_tlwhs = []
            online_ids = []
...
def main(args):
    ...
    if args.save_result:
        vis_folder = osp.join(output_dir, 'generaltrack')
