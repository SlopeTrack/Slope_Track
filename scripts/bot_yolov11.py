...
def make_parser():
    ...
    parser.add_argument("-c", "--ckpt", default='pretrained/slopetrack_yolov11.pt', type=str,
                        help="ckpt for eval")
    ...
    parser.add_argument("--fuse-score", dest="fuse_score", default=True, action="store_true",
    ...
    parser.add_argument("--fast-reid-config", dest="fast_reid_config",
                        default=r"fast_reid/configs/SlopeTrack/sbs_S50.yml", type=str, help="reid config file path")
    parser.add_argument("--fast-reid-weights", dest="fast_reid_weights", default=r"pretrained/slopetrack_sbs_S50.pth",
...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            online_targets = tracker.update(outputs, img_info['raw_img'])
            online_tlwhs = []
            online_ids = []
            online_scores = []
...
def main(args):
    ...
    if args.save_result:
        if args.with_reid:
            vis_folder = osp.join(output_dir, 'botsort_reid')
        else:
            vis_folder = osp.join(output_dir, 'botsort')
